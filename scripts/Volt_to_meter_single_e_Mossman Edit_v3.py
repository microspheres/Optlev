...
path_calibration = r"C:\data\20190408\15um\3\calibration_1e"
...
path_high_pressure_nofb = r"C:\data\20190408\15um\3"
...
def temp_path_list(pathlist, path_hp, file_hp, pathcharge, pathno, acc):
    ...
    plt.figure()
    plt.errorbar(Dgx,1e6*np.array(T),yerr=1e6*np.array(dT),fmt="ro",label="Detected Temperature")
    plt.errorbar(Dgx, 1e6 * np.array(Tbound), yerr=1e6*np.array(dT), color="blue", marker=".", linestyle="None",
