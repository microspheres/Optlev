...
if temperature and comparison:

        points1= np.logical_and(data[0] > f_start, data[0] < 59.)
        points2= np.logical_and(data[0] > 61., data[0] < 119.)
        points3= np.logical_and(data[0] > 121., data[0] < 179.)
        points4= np.logical_and(data[0] > 181., data[0] < 239.)
        points5= np.logical_and(data[0] > 241., data[0] < f_end)
        
        a = np.sum(data[1][points1])
        a = a + np.sum(data[1][points2])
        a = a + np.sum(data[1][points3])
        a = a + np.sum(data[1][points4])
        a = a + np.sum(data[1][points5])

        b = np.sum(comp[1][points1])
        b = b + np.sum(comp[1][points2])
        b = b + np.sum(comp[1][points3])
        b = b + np.sum(comp[1][points4])
        b = b + np.sum(comp[1][points5])
