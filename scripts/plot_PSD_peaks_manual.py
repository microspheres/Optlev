...
path = r"C:\data\20170829\bead12_15um_QWP_NS\dist1\dipole2"
...
def plot_peaks2F(file_list):
    ...
    for i in range(len(file_list)):
        ...
        b = get_position(dpsd)
        peaks2F[i] = xpsd[2*b] + xpsd[2*b-1] + xpsd[2*b+1] # all peak
        peaksD[i] = dpsd[b]
        f = file_list[i]
        ...
        if make_psd_plot:
                ...
                plt.plot(freqs[2*b], xpsd[2*b], "x")

#return [thetaY, thetaZ, np.sqrt(peaks2F), np.sqrt(peaksD)]
    return [np.sqrt(peaks2F), np.sqrt(peaksD)]
...
peak2W, peakD = plot_peaks2F(file_list)

plt.figure()
# plt.plot(thetaY ,peak2W)
plt.plot(np.arange(len(file_list)) ,peak2W/peakD, 'o')
# plt.plot(thetaY ,peakW/peakD, 'o')
plt.grid()
plt.show()

print np.arange(len(file_list))
