...
def save_fit_images(folder_name):
    ...
    # phase_array = np.zeros((nj, nk))
    # for j in range(nj):
    #     for k in range(nk):
    #         data = image_stack[:, j, k]
    #         curr_ind = np.argmax(data)
    #         phase_array[j, k] = -2 * radians[curr_ind]
    ...
    def show_fit_parameter(parameter_grid, title, file_name, vmax=1):
        ...
        plt.tight_layout()
        if save_images:
            plt.savefig(path_name + file_name + '_fitless.png')
        plt.show()
    ...
    # sphere_circularity_squared = sphere_circularity_squared[sphere_circularity_squared>0]
    ...
    # background_circularity_squared = background_circularity_squared[background_circularity_squared>0]
    ...
    try:
        npzfile = np.load(path_name + 'residuals_fitless.npz')
        ...
        np.savez(path_name + 'residuals_fitless.npz', residuals=residual_ar, fit=imfit)
    ...
    def plot_fit(j, k):
        ...
        fit = sinusoid(radians, p, a, c)
        sorted_radians, fit = zip(*sorted(zip(np.array(radians), np.array(fit))))
        ...
        plt.tight_layout()
        if save_images:
            plt.savefig(path_name + 'fit_plot_j' + str(j) + '_k' + str(k) + '_fitless.png')
        plt.show()
    ...
    for i in range(6):
        ...
        if debugging: plot_fit(j, k)
        else: plot_fit(j + start_j, k + start_k)


for folder in folder_name_list:
    save_fit_images(folder)

# save_fit_images(r'german_8um_vaterite_birefringence\original image analysis')
