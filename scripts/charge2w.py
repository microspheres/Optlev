...
path = r"C:\data\20170829\bead12_15um_QWP_NS\dist1\dipole"
ts = 1.
...
def getdata(fname, maxv):
	...
	print "Processing ", fname
        ...
        lentrace = len(xdat)
        ## zero pad one cycle
        MinusDC = dat[:,drive_column]*dat[:,drive_column] - np.mean(dat[:,drive_column]*dat[:,drive_column])
        corr_full = bu.corr_func(MinusDC - np.mean(MinusDC), xdat, fsamp, fdrive)
