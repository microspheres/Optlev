...
path_charge = r"C:\data\20180702\bead2_SiO2_15um_POL_NS\distances\dist16\several_voltages\calibration1e"
file_list_charge = glob.glob(path_charge+"\*.h5")

path_psd = r"C:\data\tube_test_20180906\with_tube1"
...
v_calibration = 0.15 # vpp in the daq
v_calibration = v_calibration/2.0 # now v is in amplitude

# distance = 0.0021 #m
distance = 0.005 #m

VAC = 12. # V. only used for few applications. 
...
def plot_sensitivity_electron(file_list_psd, file_list_charge): # check that the sensor gain for calibration is the same for the measurement!
    ...
    np.save(os.path.join(path_save,'freq_vs_electron_number.txt'), (A[0],c*np.sqrt(A[1])))
    plt.loglog(A[0], (np.sqrt(A[0][1]-A[0][0]))*c*np.sqrt(A[1])) # this is in electron (not 1/sqrt(Hz))
...
def plot_in_signal_units(file_list_psd, file_list_charge): # check that the sensor gain for calibration is the same for the measurement!
    ...
    plt.xlabel('Freq[Hz]')
    plt.semilogy(A[0], c*np.sqrt(A[1]))
    ...
    np.savetxt(os.path.join(path_save, label_save), (c*np.sqrt(A[1][arg]),c*np.sqrt(A[1][2*arg-1])))
    return
