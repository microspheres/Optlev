...
def corr_func(drive, response, fsamp, fdrive, good_pts=[], filt=False, band_width=1):
    ...
    if filt:
        b, a = sp.butter(3, [2. * (fdrive - band_width / 2.) / fsamp, 2. * (fdrive + band_width / 2.) / fsamp],
                         btype='bandpass')
...
def corr_blocks(drive, response, fsamp, fdrive, good_pts=[], filt=False, band_width=1, N_blocks=20):
    ...
    for i in range(N_blocks):
        corr_in_blocks[i] = \
        corr_func(drive[i * len_block:(i + 1) * len_block], response[i * len_block:(i + 1) * len_block], fsamp, fdrive,
                  good_pts, filt, band_width)[tot_phase]
